...
//                                 END;');
        ...
        DB::unprepared('DROP FUNCTION IF EXISTS coder_db.get_total_due;
                        ...
                        BEGIN
                                      ...
                                      DECLARE temp_effective_date date;
                                      DECLARE temp_closing_date date;
                                      DECLARE temp_date_differece_active int;
                                      DECLARE temp_date_difference_inactive int;

                                          select effective_date into temp_effective_date from student_to_courses where student_id = param_student_id AND course_id = param_course_id  ;
                                          select closing_date into temp_closing_date from student_to_courses where student_id = param_student_id AND course_id = param_course_id ;
                                          select inforce into temp_inforce from student_to_courses where student_id = param_student_id AND course_id = param_course_id;

                                          select ABS(TIMESTAMPDIFF(MONTH, CURRENT_DATE(), temp_effective_date)) into temp_date_differece_active;
                                          select ABS(TIMESTAMPDIFF(MONTH, temp_effective_date, temp_closing_date)) into temp_date_difference_inactive;


                                          select if(CURRENT_DATE() < temp_effective_date,0 ,if(temp_date_differece_active =0,1,if((abs(MONTH(CURRENT_DATE())- MONTH(temp_effective_date)) > 0 ) &&(DAY(temp_effective_date)-DAY(CURRENT_DATE()) > 0 ),temp_date_differece_active + 1,temp_date_differece_active))) into temp_total_month_active;

                                          select if((temp_effective_date < temp_closing_date) && (temp_date_difference_inactive = 0 ),1, temp_date_difference_inactive+1) into temp_total_month_inactive;
